...
// Cache de nomes em RAM (evita NVS a cada requisição HTTP)
static char _relayNames[8][32] = {};
static bool _relayNamesLoaded = false;

void _ensureRelayNames() {
    if (_relayNamesLoaded) return;
    _relayNamesLoaded = true;
    Preferences p;
    p.begin("app", true);
    for (int i = 0; i < 8; i++) {
        char key[16];
        snprintf(key, sizeof(key), "RelayName%d", i + 1);
        String name = p.getString(key, "");
        if (name.length() > 0) {
            strncpy(_relayNames[i], name.c_str(), sizeof(_relayNames[i]) - 1);
        } else {
            strncpy(_relayNames[i], DEFAULT_NAMES[i], sizeof(_relayNames[i]) - 1);
        }
    }
    p.end();
}
...
void handleApiSetName() {
    ...
    if (idx >= 0 && idx < 8 && newName.length() > 0) {
        ...
        _ensureRelayNames();
        strncpy(_relayNames[idx], newName.c_str(), sizeof(_relayNames[idx]) - 1);
